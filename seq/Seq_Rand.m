function [M, Seqflag,too] = Seq_Rand(tt,dt,para,M,too)

% NRF with read cst gradient
Tacq = para.Tacq;
TR  = para.TR(too.inc);
TRF = para.RF.exc.time;
ang = para.RF.exc.ang(too.inc)*pi/180;
pha = (para.RF.exc.pha0(too.inc) + para.RF.exc.phainc(too.inc))*pi/180;
dur = para.RF.exc.dur(too.inc);

% RFdir = para.RF.exc.dir(too.inc);
...
%% RF pulse
if (abs(TRF - (tt - sum(para.TR(1:too.inc-1)))) < dt *(1-1e-9))
    ...
    if too.flag.OffFlipAng
        ...
        ra(:,:) = bsxfun(@rdivide,cat(3,ang/(dur*too.gamma)*cos(pha)*ones([too.N*too.N 1]),ang/(dur*too.gamma)*sin(pha)*ones([too.N*too.N 1]),zeros([too.N*too.N 1])), ...
                            sqrt((ang/(dur*too.gamma)*ones([too.N*too.N 1])).^2 + zeros([too.N*too.N 1]).^2));
    ...
    RFc = para.RF.exc.ang(too.inc-1)*pi/180;
    Phic = (para.RF.exc.pha0 + para.RF.exc.phainc(too.inc-1))*pi/180;
end


if any( abs(Tacq - (tt - sum(para.TR(1:too.inc-2))))< dt *(1-1e-9))
    Seqflag.Acq = 1;
else
    Seqflag.Acq = 0;
end
...
too.seq.RFphi   = [too.seq.RFphi Phic];
